...
def name_error(test_py) :
  print(error_line(), 'line.There is an undefined variable on line number.')
  print('Or is it an unquoted string? Or perhaps a command entered incorrectly')
  print("="*40)
  code_print(test_py)

def type_error(test_py) :
  print(error_line(), 'line. Did you enter the data in the correct format?')
  print("="*40)
  code_print(test_py)

def attribute_error(test_py) :
  print(error_line(), 'line. Did you correctly enter the property or method of the library?')
  print("="*40)
  code_print(test_py)

def value_error(test_py) :
  print(error_line(), 'line. Did you enter the appropriate value?')
  print("="*40)
  code_print(test_py)

def index_error(test_py) :
  print(error_line(), 'line. You have exceeded the length of the sequence data type (such as a list).')
  print("="*40)
  code_print(test_py)

def key_error(test_py) :
  print(error_line(), 'line. The key cannot be found in the dictionary.')
  print("="*40)
  code_print(test_py)


def indentation_error(test_py) :
  print(error_line(), 'Please check the spacing.')
  print("="*40)
  code_print(test_py)

def zerodivision_error(test_py) :
  print(error_line(), 'line. You cannot divide a number by zero.')
  print("="*40)
  code_print(test_py)

def overflow_error(test_py) :
  print(error_line(), 'line. The number is too large to be represented.')
  print("="*40)
  code_print(test_py)

def keyboard_interrupt(test_py) :
  print('The user has stopped the operation.')
...
def import_error(test_py) : 
  print(error_line(), 'line. Please check the library you are trying to import.' )
  print("="*40)
  code_print(test_py)

def else_error(test_py) :
  print(error_line(), 'line. There is an Error')
  print("="*40)
  code_print(test_py)
...
def error_check(test_py) :
  ...
  try :
    ...
    type_error(test_py)
    compile_error = True
    print('Error Message : ',bc_black+str(typeerror)+reset)
    #에러 메세지 txt 파일로 저장
    #여기서 오류가 난 코드와 원인 출력
    sys.stdout = original
    ...
    value_error(test_py)
    compile_error = True
    print('Error Message : ',bc_black+str(valueerror)+reset)    
...
def table_check(py) :
  ...
  # 결과 자가 평가
    ...
    if df.shape == df_answer.shape : 
      global table_count
      table_count = 0     
      df_answer_html = df.style.format(precision = 2). to_html(max_rows = 5, max_columns = 5, show_dimensions = True).replace('<table', '<table class = "dataframe"')
      df_html_color = df.style.format(precision=2).applymap(table_compare, df_answer = df_answer).to_html(max_rows = 5, max_columns = 5, show_dimensions = True).replace('<table', '<table class = "dataframe"')
      output_html_color = f'''
